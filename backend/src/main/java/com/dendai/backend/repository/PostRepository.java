...
public interface PostRepository extends JpaRepository<Post, Long> {
        ...
        @Query(value = "SELECT p.post_id AS postId, p.title, p.description, " +
                        ...
                        "GROUP BY p.post_id, u.username " +
                        "ORDER BY likesCount DESC, createdAt DESC", countQuery = "SELECT COUNT(DISTINCT p.post_id) FROM Posts p "
                                        +
                                        "WHERE (:keyword IS NULL OR LOWER(p.title) LIKE LOWER(CONCAT('%', :keyword, '%')) "
                                        +
                                        "    OR LOWER(p.description) LIKE LOWER(CONCAT('%', :keyword, '%'))) " +
        ...
        @Query(value = "SELECT p.post_id AS postId, p.title, p.description, " +
                        "p.academic_year AS year, p.department, p.grade, " +
                        "p.related_period AS semester, p.created_at AS createdAt, p.updated_at AS updatedAt," +
                        "u.username, COUNT(DISTINCT b.post_id) AS likesCount, p.user_id AS userId, " +
                        "array_agg(DISTINCT s.link) AS sharedUrls " +
                        "FROM Posts p " +
                        "LEFT JOIN Bookmarks b ON p.post_id = b.post_id " +
                        "LEFT JOIN Users u ON p.user_id = u.user_id " +
                        "LEFT JOIN SharedURLs s ON p.post_id = s.post_id " +
